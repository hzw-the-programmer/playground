...
// curl -X POST localhost:3000/derive-from-request -H 'Content-Type: application/json' -d '{"login":"my_login","password":"my_password"}'
// curl --data-urlencode "name=Rafael Sagula&phone=3320780" localhost:3000/derive-from-request

pub async fn handler(Json(value): Json<Value>) -> impl IntoResponse {
    println!("derive_from_request: handler");
    Json(dbg!(value))
}
...
impl From<JsonRejection> for ApiError {
    fn from(rejection: JsonRejection) -> Self {
        println!("derive_from_request: from");
        Self {
...
impl IntoResponse for ApiError {
    fn into_response(self) -> axum::response::Response {
        println!("derive_from_request: into_response");
        let payload = json!({
