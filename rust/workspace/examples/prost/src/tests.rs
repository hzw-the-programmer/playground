...
fn test_submsg2() {
    ...
    // SubMsg2 encode
    let t = submsg::SubMsg2 { a: None };
    let mut buf = Vec::new();
    buf.reserve(t.encoded_len());
    t.encode(&mut buf).unwrap();
    assert_eq!(hex::encode(&buf), "");

    let t = submsg::SubMsg2 { a: Some(0) };
    let mut buf = Vec::new();
    buf.reserve(t.encoded_len());
    t.encode(&mut buf).unwrap();
    ...
    // SubMsg decode
    let t = submsg::SubMsg::decode(&hex::decode("").unwrap()[..]).unwrap();
    assert_eq!(t.a, 0);
    let t = submsg::SubMsg::decode(&hex::decode("0800").unwrap()[..]).unwrap();
    assert_eq!(t.a, 0);
    let t = submsg::SubMsg::decode(&hex::decode("0801").unwrap()[..]).unwrap();
    assert_eq!(t.a, 1);

    // SubMsg2 decode
    let t = submsg::SubMsg2::decode(&hex::decode("").unwrap()[..]).unwrap();
    assert_eq!(t.a, None);
    let t = submsg::SubMsg2::decode(&hex::decode("0800").unwrap()[..]).unwrap();
    assert_eq!(t.a, Some(0));
    let t = submsg::SubMsg2::decode(&hex::decode("0801").unwrap()[..]).unwrap();
