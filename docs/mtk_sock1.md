...
MMI_BOOL gRunInBg = MMI_FALSE;
kal_uint16 gText[100] = {0};
kal_int16 gTextY = 0;
...
static void hzwUpdate(kal_char *text) {
    kal_wsprintf(gText, "%s", text);
    hzwDraw();
}

kal_uint32 gEncodedAccountId = 0;

static void hzwDnsCb() {
}

static MMI_BOOL hzwDnsNotify(void *msg) {
    app_soc_get_host_by_name_ind_struct *dns_msg = (app_soc_get_host_by_name_ind_struct*)msg;
    ...
    hzwUpdate("finish,block");
    hzwDnsCb();
}

typedef struct HzwConnection {
    kal_int8 sock;
    MMI_BOOL inUse;
    kal_uint32 createdAt;
    struct HzwConnection *next;
} HzwConnection;

static HzwConnection *gConns = NULL;

static MMI_BOOL hzwSocNotify(void *msg) {
    app_soc_notify_ind_struct *soc_msg = (app_soc_notify_ind_struct*)msg;
    kal_int32 ret = 0;
    const kal_char req[] = "GET / HTTP/1.1\r\n\r\n\r\n";

    if (!soc_msg) {
        return MMI_FALSE;
    }

    switch (soc_msg->event_type) {
        case SOC_CONNECT: {
            ret = soc_send(soc_msg->socket_id, req, strlen(req), 0);
            break;
        }
        case SOC_WRITE: {
            break;
...
kal_uint32 hzwEncodedAccountId() {
    kal_uint32 acct_id = 0;
    cbm_sim_id_enum sim_id = CBM_SIM_ID_SIM1;
    kal_uint8 app_id = 0;
    kal_bool always_ask = KAL_FALSE;

    if (cbm_get_valid_account_id(CBM_GPRS, &acct_id) != CBM_OK) {
        return 0;
    }

    if (cbm_register_app_id(&app_id) != CBM_OK) {
        return 0;
    }

    return cbm_encode_data_account_id(acct_id, sim_id, app_id, always_ask);
}
...
void hzwDns(const kal_char *name) {
    kal_int8 ret = 0;
    kal_uint8 addr[8] = {0};
    kal_uint8 addrLen = 8;
    ...
    ret = soc_gethostbyname(KAL_FALSE, 
                            ...
                            gEncodedAccountId);
    if (ret == SOC_SUCCESS) {
        hzwUpdate("finish,no block");
        hzwDnsCb();
    }
}

void hzwConnect(kal_uint8 *buf, kal_int16 len, kal_uint16 port) {
    kal_int8 sock = 0;
    kal_uint8 val = 1;
    sockaddr_struct addr;

    sock = soc_create(SOC_PF_INET, SOC_SOCK_STREAM, 0, MOD_MMI, gEncodedAccountId);
    if (sock < 0) {
        return;
    }
    ...
    val = SOC_CONNECT | SOC_WRITE | SOC_READ | SOC_CLOSE;
...
void hzwKeyHandler() {
    ...
    switch (keyCode) {
        case KEY_RSK:
        case KEY_END:
            gRunInBg = keyCode == KEY_END;
            mmi_frm_group_close(HZW_GRP_ID);
            break;

        case KEY_1:
            hzwDns("www.baidu.com");
            break;
