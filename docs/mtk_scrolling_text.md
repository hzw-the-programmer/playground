...
void hzwKeyHandler() {
	...
	if (keyCode == KEY_END && keyType == KEY_EVENT_UP) {
		mmi_frm_group_close(HZW_GRP_ID);
	}
}

static void hzwSetKeyHandler() {
...
static scrolling_text gST = {0};

static void hzwScrollingTextTimerCb() {
	gui_handle_scrolling_text(&gST);
}

static void hzwScrollingTextDrawBg(S32 x1, S32 y1, S32 x2, S32 y2) {
	gdi_draw_solid_rect(x1, y1, x2, y2, GDI_COLOR_WHITE);
}

static void hzwExitProc() {
	gui_scrolling_text_stop(&gST);
}
...
static void hzwEntryProc() {
	...
	gui_create_scrolling_text(
		...
		50,
		L"",
		hzwScrollingTextTimerCb,
		hzwScrollingTextDrawBg,
		gui_color(255, 255, 255),
		gui_color(0, 0, 0)
	);
	gui_change_scrolling_text_font(&gST, &MMI_medium_font);
	gST.flags |= UI_SCROLLING_TEXT_BORDERED_TEXT;
	gui_change_scrolling_text(&gST, L"I am a scrolling text!!!");
	gui_scrolling_text_start(&gST);
}
