...
void hzwDraw() {
	...
	UI_string_type lstr = L"abcdefghijklmnopqrstuvwxyz"
		L"abcdefghijklmnopqrstuvwxyz"
		L"abcdefghijklmnopqrstuvwxyz"
		L"abcdefghijklmnopqrstuvwxyz";
	S32 lstrLen = gui_strlen(lstr);
	S32 x = 0, y = 0, w = 0, h = 0;
	...
	gui_create_multi_line_input_box_set_buffer(&b, x, y, w, h, lstr, (lstrLen + 1) << 1, (lstrLen + 1) << 1, 0);
	gui_set_multi_line_input_box_default_bg_color(&b, gui_color(255, 0, 0), MMI_TRUE);
	b.flags |= UI_MULTI_LINE_INPUT_BOX_DISABLE_CURSOR_DRAW | UI_MULTI_LINE_INPUT_BOX_DISABLE_SCROLLBAR;
	x = 10; y = 50;
	gui_move_multi_line_input_box(&b, x, y);
	w = UI_DEVICE_WIDTH - 2 * x;
	h = UI_DEVICE_HEIGHT - 2 * y;
	gui_resize_multi_line_input_box(&b, w, h);
