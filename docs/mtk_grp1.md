```
#define HZW_GRP_ID 100
#define HZW_SCR_ID 1000

static void hzwEntryProc(mmi_scrn_essential_struct* scrn);
static mmi_ret hzwScreenProc(mmi_scenario_evt_struct* evt);
...
static char* hzwEvtStr(U16 evtId) {
	switch (evtId) {
        ...
        case EVT_ID_PHB_SAVE_CONTACT_CANCEL:
            return "EVT_ID_PHB_SAVE_CONTACT_CANCEL";
        case EVT_ID_PHB_SAVE_CONTACT:
            return "EVT_ID_PHB_SAVE_CONTACT";
		default:
			return "DEFAULT";
	}
}
...
void hzwDraw(UI_string_type ws) {
...
void hzwKeyHandler() {
    ...
    scrnId = mmi_frm_scrn_get_active_id();
    ...
    switch (keyCode) {
        ...
        case KEY_3: {
            mmi_id id = 0;
            id = cui_phb_save_contact_create(HZW_GRP_ID);
            cui_phb_save_contact_set_name(id, L"Zw", L"H");
            cui_phb_save_contact_set_number(id, L"0123456");
            cui_phb_save_contact_run(id);
            break;
        }
    }
}
...
static mmi_ret hzwGrpProc(mmi_event_struct* evt) {
    ...
    switch (evt->evt_id) {
        case EVT_ID_PHB_SAVE_CONTACT_CANCEL:
        case EVT_ID_PHB_SAVE_CONTACT: {
            cui_phb_save_contact_result_struct *result = (cui_phb_save_contact_result_struct*)evt;
            cui_phb_save_contact_close(result->sender_id);
            break;
        }
    }
...
static void hzwExitProc(mmi_scrn_essential_struct *data) {
	mmi_id grpId = data->group_id;
    mmi_id scrnId = data->scrn_id;
    void *userData = data->user_data;
}

static mmi_ret hzwScreenProc(mmi_scenario_evt_struct* evt) {
	char *evtStr = hzwEvtStr(evt->evt_id);

    MMI_ASSERT(123 == evt->user_data);

	return MMI_RET_OK;
}

static void hzwEntryProc(mmi_scrn_essential_struct* scrn) {
    UI_character_type ws[100] = {0};
    ...
    kal_wsprintf(ws, "%d", scrn->scrn_id);
	hzwDraw(ws);
