...
static int format_decode(const wchar_t *fmt, struct printf_spec *spec) {
...
static void set(wchar_t *buf, wchar_t wc, int size) {
	for (int i = 0; i < size; i++) {
		buf[i] = wc;
	}
}

static void move(wchar_t *buf1, wchar_t *buf2, int size) {
	for (int i = 0; i < size; i++) {
		buf1[i] = buf2[i];
	}
}
...
static void ws_append_s(wchar_t *ws, const char *s, int len) {
	for (int i = 0; i < len; i++) {
		ws[i] = s[i];
	}
}

static void ws_append_ws(wchar_t *ws, const wchar_t *s, int len) {
	for (int i = 0; i < len; i++) {
		ws[i] = s[i];
	}
}
