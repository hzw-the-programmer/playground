...
class ChannelsStatus extends Task {
    static $cntx;
    static $sqls = [
        'EXEC new_channel_status :sn, :slot, :port, :type, :dt, :seq, :pcdt, :status, :data',
    ];
    static $stmts = [];
    static $className;
    ...
    function onBackup($f) {
        ...
        foreach ($channels as $channel) {
            fputcsv($f, [
                $channel->getSlot(), $channel->getPort(), $channel->getType(),
                $channel->getStatus(), $channel->getData()
            ]);
        }
        fputcsv($f, [$className]);
    }
    ...
    static function onRestore($f) {
        ...
        while (($fields = fgetcsv($f)) && $fields[0] !== $className) {
            list($slot, $port, $type, $status, $data) = $fields;
        ...
        return new self($sn, $seq, $dt, $pcdt, $channels);
    }
}
